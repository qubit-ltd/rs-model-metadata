...
fn test_field_occurrences_preserve_validator_order_and_declared_codec_id() {
    ...
    assert!(
        owner
            .indexing_reasons()
            .contains(model_runtime::IndexingReasons::REFERENCE)
    );
...
fn test_enum_and_value_role_payloads_use_reflection_overlays() {
    ...
    let redacted = model_runtime::__private::qubit_redact::Redactor::application_default().redact(&email);
    ...
    let redacted = model_runtime::__private::qubit_redact::Redactor::application_default().redact(&secret);
    ...
    let redacted = model_runtime::__private::qubit_redact::Redactor::application_default().redact(&payload);
...
fn test_generic_model_registers_only_its_definition() {
    ...
    assert!(matches!(
        ...
        Some(model_runtime::expression::TypeExpression::Parameter(name)) if name.as_ref() == "T",
    ));
