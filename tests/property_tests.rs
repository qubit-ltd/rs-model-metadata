...
fn test_property_supports_borrowed_and_owned_getters() {
    ...
    let name =
        v4::property_metadata("name", name_type, None, Some(name_getter), None);
    let count = v4::property_metadata(
        "count",
        count_type,
        None,
        Some(count_getter),
...
fn test_property_rejects_wrong_targets_and_field_fallback_can_write() {
    ...
    let computed = v4::property_metadata(
        "name",
        field.type_ref(),
        None,
        Some(getter),
    ...
    let fallback = v4::property_metadata(
        "name",
        field.type_ref(),
        Some(field),
        None,
