...
Multiple ModelImpl blocks register independent checked capabilities. Aggregate only providers visible in the supplied reflection snapshot and validate their owner and accessors. Direct `TypeMetadata::*_in` calls return owned `ResolvedProperties` and `ResolvedPropertyFragments`; they do not populate a process-wide cache. `ModelRegistry` caches successful results and assembly failures by `(TypeId, TypeMetadata pointer)` inside that registry, so dropping an explicit registry releases its dynamic merges. Generic method contributions use reflect_impl explicit specializations.

`ResolvedProperties::Static` refers to generated static metadata; `Merged` owns an `Arc<[PropertyMetadata]>`. `try_property[_in]` copies the `Copy` property record. `ModelGraph` owns the resolved property views and returns borrows tied to the graph. Assembly errors own their diagnostics through `Arc<PropertyBuildErrors>`. Snapshot-specific properties, fragments, cache cells, and errors are not leaked for process lifetime.
