...
targets the versioned hidden facade of `qubit-model-metadata`, preserving the
runtime ABI and renamed-dependency support.
...
parse ──► declaration IR ──► normalize ──► validate ──► expand
                                  ...
                                  Reflect descriptor + TypeMetadata capability
                                                        │
                                                        ▼
                                    optional link-time registration fragments
```
...
- `expand/{declaration,metadata,fields,role,capabilities,type_expression,
  registration,model_impl}.rs` emits focused code-generation products;
...
Static metadata access does not touch a global registry. Link-time fragments
register only declarations with stable IDs. Generic declarations register one
symbolic definition, while each concrete type retains its ordinary reflection
descriptor and links to that definition.
