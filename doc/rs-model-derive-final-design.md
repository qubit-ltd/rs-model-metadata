...
targets model ABI v4 in the versioned hidden facade of
`qubit-model-metadata`, preserving renamed-dependency support through a
curated set of exact public and private exports.
...
parse ──► declaration IR ──► normalize ──► validate ──► expand
                                  ...
                                  Reflect descriptor + TypeMetadata capability
                                                        │
                                                        ▼
                         unified concrete + definition reflection fragments
```
...
Concrete declarations and generic definitions both attach typed capabilities
to the one `ReflectRegistry`. `ModelRegistry` projects those capabilities and
their authoritative provenance; it owns no inventory or registration stream.
Each concrete generic type keeps its runtime descriptor and links to the
first-class symbolic `TypeDefinitionDescriptor`.
