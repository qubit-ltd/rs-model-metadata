...
出的 `#[serde(...)]`。宏只识别无限定预导入写法及显式标准库路径，不处理类型别名或
末段名称相同的限定路径自定义类型。若无限定自定义类型遮蔽了标准名称，过程宏目前无法
区分；请添加 `#[keep_serializing]`，以保留该类型自身的 Serde 行为。固定长度数组只有
长度为零时才为空，长度非零的数组仍会输出。
...
tuple 载荷的 Serde 仍按位置编码。对于至少有两个字段的 tuple variant，自动省略只会
作用于最后一个可选字段或空集合字段；反序列化缺失的尾部字段时使用 `default`。单字段
newtype variant 受 Serde 表示限制，会保留 `null` 或空集合。更靠前的 tuple 字段即使
为空也会保留在输出中，因此后续字段不会发生左移。
