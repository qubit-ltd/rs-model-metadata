...
> **面向智能体执行者：** 必须使用 superpowers:subagent-driven-development（推荐）或 superpowers:executing-plans，逐项实施本计划。各步骤使用复选框（`- [x]`）语法跟踪进度。
...
- 本计划制定时只授权文档；2026-09-10 用户已授权全部实施、创建 worktree、分组英文提交、快进合并与推送，并明确不使用 subagent。
...
| --- | --- | --- | --- | --- | --- | --- |
| T1 共享表示与 ABI | 无 | v6 路径/依赖/可选泛型 ID/解析根接口可编译 | src/relation/、metadata_vocabulary.rs、generic/generic_model_metadata.rs、resolve/resolver.rs 输入、__private.rs、private/、derive pipeline 分发、tests/abi_v6_tests.rs、abi_checked_contract_tests.rs | cargo test -p qubit-model-metadata --all-features --test abi_v6_tests | T8 | 立即 |
...
- [x] 新建 `rs-model-metadata/tests/abi_v6_tests.rs` 与 `abi_checked_contract_tests.rs`，分别验证属性/Parent 步骤保持、空步骤拒绝、可选 ID、错误来源及 checked descriptor/accessor 不匹配；使用真实不匹配类型，不能只比较版本常量。
- [x] 运行 T1 本地命令，记录新增目标接口不存在或旧类型不匹配的预期失败。
- [x] 实现 ObjectPath 的强类型步骤和声明位置；PropertyPath 仍用于纯属性选择；不允许把 Parent 作为名称存入 PropertyPath。
- [x] 发布 checked v6 构造和受控 facade 重导出；新增接口暂不改变消费者算法，调整现有构造调用使 workspace 可编译。
- [x] 泛型定义 model_id 改为 Option；定义有无 ID 的构造均合法。ResolveInputs 新增 roots，旧调用处显式给空切片作为过渡。
- [x] 运行 T1 命令和 `cargo check --workspace --all-features`，审查公共接口、布局和借用边界；通过后解锁 T2/T3。
...
- [x] 新建 default_output_tests，先加入未手写 derive 的角色例子；为输出断言增加明确的 serde_json 测试依赖和输出 runtime 依赖。
...
- [x] 运行 T2 命令，确认因缺少默认 trait 失败，不是因为 fixture 缺少导入。
- [x] 实现能力矩阵、显式 derive 去重、准确 bound 和 Eq/Hash 联动；加入 float+opt-out、unit Enum Copy、Enum default、泛型非必要 bound 的正反例。
- [x] 委托 rs-redact 生成字段投影，自动 Debug/Display/Serialize 通过借用视图输出；Deserialize 委托原 Serde 语义。
- [x] 加入 secret 不泄漏、skip 全字段形状、disabled 恢复、map key 冲突、透明 Value、显式 serializer 冲突和 no_redact 嵌套输出测试；测试必须断言字符串/JSON 内容或结构化错误。
- [x] 加入 named Option/集合自动 omit/default、keep_serializing 与显式 skip 的优先级测试。位置字段只有显式 skip 能省略。
- [x] 运行 T2 命令；逐条审查输出接入，避免递归调用自身 Serialize 或先编码 JSON 字符串再序列化。
...
- [x] 新建 model_impl_contract_tests，包含普通方法、trait impl、带 where 的泛型 impl、不可动态调用方法和以下排除场景。
...
- [x] 运行 T3 命令确认旧 impl 限制或未知标记导致预期失败。
- [x] 先剥离 model_property(skip)，将完整 impl 委托 reflect_impl；Property 候选只来自未排除的 inherent 方法。
- [x] 复用原兼容类型映射及反射动态访问，对 getter/setter 冲突按来源确定排序；不符合 Property 形状只保留反射。
- [x] 验证同名 field 不因 getter skip 消失，trait 方法可反射但不贡献 Property；借用结果不能逃逸，Local 实例不被要求 Send。
- [x] 运行两个 T3 测试目标，审查动态访问和泛型 impl fragment 的 TypeId 隔离。
...
- [x] 新建 declaration_semantics_tests，输入两个同 ID validator、一个局部依赖和一个 parent 依赖，断言 occurrence 数量/顺序和结构化步骤。
- [x] 添加 Enum 中 Id reference 的通过 fixture、Value 经 Enum 隐藏 reference 的失败 fixture；检查 variant/index 定位。
- [x] 运行 T4 命令确认现有点路径/Enum 禁令等引发失败。
- [x] 分离 ObjectPath 与 PropertyPath parser；实现 `depends_on(slot(path = "..", property = birthday))` 及裸依赖简写，混合简写和结构化形式规范化到同一 IR。
- [x] UniqueIr 保留显式 ignore_case；有效默认以 text capability 决定。真实 Id 别名沿用 sealed bound，补防回归测试，不换成字符串启发式。
- [x] 保留重复 indexed、Set unique_items、数组 min/max 和角色非法组合诊断；删除 target/on_none/validate_nested 未采纳语法的默认入口，并给迁移诊断。
- [x] 约束记录类型/使用位置来源；decimal 相等端点检查空区间；不在宏求解任意约束组合。
- [x] 运行 T4 命令，审查标准约束和 selector 的作用位置，不把普通容器当透明包装。
...
- [x] 新建 structure_roots_tests，包含无 ID Model、无 ID 泛型 concrete、两个根共享嵌套类型、递归 Entity 引用、Option<Vec<Id>> reference。
...
- [x] 运行 T5 命令，记录旧 metadata 缺失关联或没有匿名解析的失败。
- [x] 为所有泛型生成 definition provider；仅 Some(id) 进入模型稳定 ID 枚举。
- [x] 从 registry 与 roots 并集遍历闭包；按 TypeId 去重，所有 enum payload 参与，FieldLocation 使用 variant/index。
- [x] 引用字段先按精确兼容匹配，再逐层穿透允许包装，记录外层结构；Value 闭包按已确认规则检查。
- [x] Parent 依赖保留 RequiresContext；结构类型已知的部分正常检查，未知父实例不视作非法声明。
- [x] 从 queries 移出 filter 展开/平面名检查；将 unique scope 检查迁到 relations；断言旧 flat-name 冲突模型可 resolve，而不存在 scope 属性仍失败。
- [x] 多线程重复查询验证同 TypeId 共享 metadata，错误不变为空结果；运行 T5 命令并审查缓存锁与递归终止。
...
- [x] 扩展已有三组测试：无 ID root 不 panic；同 ID validator 不同参数分别执行；父依赖由显式上下文提供，缺失时返回带路径错误。
- [x] 运行 T6 命令，确认无 ID expect 或旧局部路径编译暴露缺口。
- [x] 将计划/错误身份改为 TypeId + Option<ModelId>；移除必填 ID expect，不使用假 ID 兼容旧函数。
- [x] 依赖编译拆为已解析前缀与上下文导航；只有具备输入类型时才绑定依赖类型，否则记录未满足的上下文条件。
- [x] 按既定 Option/selector/opaque 边界执行，类型约束与使用位置约束均保留；不写回被验证值。
- [x] Codec 增加显式等于 canonical、匿名可达模型、准确类型错误测试；绑定仍显式使用传入 registry。
- [x] 运行 T6 命令；审核本库只是执行协议适配，没有加入数据库/随机对象/产品 filter 算法。
...
- [x] 按文件/角色重采集声明基线，与审计的 131 条比对；逐项记录 cfg/新增删除造成的差异。
- [x] 先处理 gap 报告列出的 5 个点路径候选文件，只替换 reference.path 中的分隔符。
- [x] 清理与新自动 trait 冲突的 derive；有手写实现的地方显式 no_*，不删除有业务含义的手写实现。
- [x] 将旧依赖语法和未采纳开关改为新声明或下游配置；替换对 filters/flat_name 的调用，但不为迁移而新建未授权查询产品。
- [x] 更新 runtime fixtures、trybuild 旧拒绝能力测试和指南；将历史状态说明改为真实支持范围。反向测试必须变为新的正向/负向契约测试。
- [x] 在 rs-model-metadata 目录运行 T7 cargo check；定位实际失败并修复属于本任务的迁移。外部依赖缺陷单独记录，不放宽新需求。
...
- [x] 真实 runtime-fixtures 检查重命名 runtime、缺失 runtime、跨 crate ID/source/reference 和策略绑定；使用已有 fixture runner，不以同 crate 测试替代。
- [x] 逐项为台账添加实现路径、测试名/文档审查记录、命令和执行日期；对未满足条目保持未完成，不把抽样成功套到整组。
- [x] 审查默认输出安全、匿名/泛型图、borrowed getter 和 ABI，确认无 panic 代替缺失/错误的路径。
- [x] 对齐需求、新设计、API rustdoc、指南和迁移基线。候选查询方案及外部算法仍标下游，不计本库失败项。
- [x] 交付变更、测试结果和剩余外部限制；Git 集成按本轮用户授权在计划验收后执行。
...
- 用户授权串行实施，不使用 subagent。T1～T7 已逐项实现并运行对应目标测试。
- T1 的路径/可选 ID 测试在 `abi_v6_tests`，原 checked ABI 契约迁移至 `abi_checked_contract_tests`；不是删除安全断言。
- T2 除本库自动能力生成外，修复 rs-redact 对 deserialize 专用 bound 与递归投影的支持；递归泛型同时验证 Clone/Eq/Debug/Serialize/Deserialize。
- T3 除借用输出兼容外，修复 rs-reflect 的 unsized borrowed output 分类；增加多个 ModelImpl block 的合并测试。
- T5 在原计划基础上补验 Enum 无引用 Entity payload、validator 实际存储对象导航及 Option/Box 点分属性选择。
