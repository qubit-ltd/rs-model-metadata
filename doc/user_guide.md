...
- `ModelResolveErrors` aggregates missing IDs, wrong roles, missing or
  unreadable properties, invalid projection sources, and missing or
  value-type-incompatible validator/codec registrations.
...
- A `QMM-ABI-*` panic means generated or manually supplied hidden-ABI metadata
  violated a local invariant and was rejected before publication.
