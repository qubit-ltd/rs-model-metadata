...
use crate::attribute::{FieldName, SpannedValue, TextAttribute};
use crate::normalize::{
    DecimalIr, DecimalSemantic, ElementConstraintIr, ElementIr, FieldAttributeIr, FieldIr,
    ModelAttributeIr, ModelIr, ModelShapeIr, NamedFieldsIr, PrimaryKeyIr, UniqueIr,
};
...
fn validate_field_attribute(
    ...
    match attribute {
        ...
        FieldAttributeIr::Sensitive(value) => {
            validate_duplicate_values("sensitive handling", &value.handling, errors);
        }
...
fn validate_strategy_name(kind: &str, names: &[syn::LitStr], errors: &mut Option<Error>) {
...
fn validate_duplicate_literals(name: &str, values: &[syn::LitStr], errors: &mut Option<Error>) {
    for value in values.iter().skip(1) {
        push_error(
            errors,
            Error::new(value.span(), format!("duplicate `{name}` argument")),
...
fn validate_duplicate_type_paths(name: &str, values: &[syn::TypePath], errors: &mut Option<Error>) {
    for value in values.iter().skip(1) {
        push_error(
            errors,
            Error::new(value.span(), format!("duplicate `{name}` argument")),
