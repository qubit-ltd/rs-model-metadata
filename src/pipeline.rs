...
//! Shared parse, validation, and expansion pipeline for every public macro.
...
/// Selects the public macro while retaining one shared implementation.
...
fn expand_result(kind: MacroKind, args: TokenStream, input: TokenStream) -> Result<TokenStream> {
    ...
    if kind == MacroKind::ModelProperties {
        ...
        validate_property_impl(&item)?;
        let runtime = runtime_path()?;
        return expand_properties(item, &runtime);
    }
...
fn validate_property_impl(item: &ItemImpl) -> Result<()> {
...
fn expand_properties(item: ItemImpl, runtime: &TokenStream) -> Result<TokenStream> {
